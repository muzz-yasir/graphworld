...
def NetworkitToGraphWorldData(G, lfr_data, community_sizes):
  """
  Converts NetworkX graph data to GraphWorld LFR dataclass with node memberships.
  Args:
...
def SimulateLFR(
  ...
  Attemps to simulate the graph multiple times before moving on.
  Args:
    n: number of nodes 
    ...
    mu: mixing parameter
  Returns:
    lfrG: the generated NetworKit LFR graph
    community_sizes: sequence of community sizes in the generated graph
  """
  for i in range(3):
    try:
      ...
      print(e)
      continue
  print(f'n={n}, min/max community = {min_community_size}/{max_community_size}, max deg={max_deg}, mu = {mu}')
...
def GenerateLFRGraphWithFeatures(
    ...
    """
    lfr_model, community_sizes = SimulateLFR(n=n, 
                                            avg_deg=avg_deg,
                                            max_deg=max_deg,
                                            exponent=exponent,
                                            min_community_size=min_community_size,
                                            max_community_size=max_community_size,
                                            exponent_community=community_exponent,
                                            mu=mixing_param)     
    result = LFR()
