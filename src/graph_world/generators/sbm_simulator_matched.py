...
def _ComputePowerLawCommunitySizes(num_vertices, min_community_size, max_community_size, exponent, out_degs):
  """
  ...
  generator.generatePowerlawDegreeSequence(15,int(0.75*0.2*num_vertices),-3) 
    #generator.setDegreeSequence(out_degs) 
  generator.setMu(0.5)
  generator.run()
...
def _GenerateNodeMemberships(n, community_sizes):
  """
  Generates Node Memberships based on community sizes.
  Args:
...
def SimulateSbm(sbm_data,
                num_vertices,
                num_edges, min_community_size, max_community_size, exponent,
                p_to_q_ratio, 
                out_degs=None):
  ...
  community_sizes = _ComputePowerLawCommunitySizes(num_vertices, min_community_size, max_community_size, exponent, out_degs)
  
  pi = _MakePi(community_sizes)
  prop_mat = _MakePropMat(len(community_sizes), p_to_q_ratio)
  ...
  sbm_data.graph_memberships = _GenerateNodeMemberships(num_vertices, community_sizes)
...
def GenerateStochasticBlockModelWithFeatures(
    num_vertices,
    num_edges,
    min_community_size, max_community_size, community_exponent, p_to_q_ratio,
    out_degs=None,
  ...
  SimulateSbm(result,
              num_vertices,
              num_edges, min_community_size, max_community_size, community_exponent,
              p_to_q_ratio, out_degs)
...
def _MakePi(community_sizes) -> np.ndarray:
  community_sizes = np.array(community_sizes)
  pi = community_sizes / sum(community_sizes)
  return pi
...
def _MakePropMat(num_communities: int, p_to_q_ratio: float) -> np.ndarray:
